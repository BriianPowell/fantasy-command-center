...
export function scoreDraftPlayerValue(player: Player, ranking?: Ranking, projection?: Projection): number {
  const rankScore = ranking ? Math.max(0, 120 - ranking.rank) : scoreSleeperSearchRank(player.searchRank);
