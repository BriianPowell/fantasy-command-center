...
export class SleeperApi {
  ...
  private trendingPlayersCache = new Map<string, Promise<SleeperTrendingPlayer[]>>();
  ...
  fetchLeagueMatchups(leagueId: string, week: number): Promise<SleeperMatchup[]> {
    return sleeperFetch<SleeperMatchup[]>(`/league/${leagueId}/matchups/${week}`);
  }

  fetchTransactions(leagueId: string, week: number): Promise<SleeperTransaction[]> {
  ...
  fetchTrendingPlayers(type: "add" | "drop", options: SleeperTrendingOptions = {}): Promise<SleeperTrendingPlayer[]> {
