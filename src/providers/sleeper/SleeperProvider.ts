...
export class SleeperProvider {
  ...
  async loadDraft(draftId: string): Promise<DraftState> {
    const [draft, picks] = await Promise.all([this.api.fetchDraft(draftId), this.api.fetchDraftPicks(draftId)]);
  ...
  async loadLeagueMatchups(leagueId: string, week: number): Promise<LeagueMatchup[]> {
    const matchups = await this.api.fetchLeagueMatchups(leagueId, week);

    return matchups.map(normalizeMatchup);
  }

  async loadTransactions(leagueId: string, week: number): Promise<LeagueTransaction[]> {
  ...
  async loadTrendingPlayers(
    ...
    return trendingPlayers.map((player) => normalizeTrendingPlayer(player, type));
  }
