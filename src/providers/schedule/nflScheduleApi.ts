...
export async function loadNflTeamByeWeeks(
  season: string
...
async function fetchNflTeamByeWeeks(
  season: string
): Promise<Record<string, number>> {
  const weeklyTeams = await Promise.all(
    REGULAR_SEASON_WEEKS.map((week) => fetchWeekTeams(season, week))
  )
...
function validateByeWeeks(byeWeeks: Partial<Record<NflTeam, number>>) {
  ...
  if (missingTeams.length) {
    throw new Error(
      `Incomplete NFL bye-week schedule. Missing teams: ${missingTeams.join(', ')}`
    )
  }
}
...
async function fetchWeekTeams(
  ...
  if (!response.ok) {
    throw new Error(
      `NFL schedule request failed: ${response.status} ${response.statusText}`
    )
  }
