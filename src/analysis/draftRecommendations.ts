...
export function buildDraftRecommendations(input: DraftRecommendationInput): DraftRecommendation[] {
  const rankingByPlayer = new Map(input.rankings.map((ranking) => [ranking.playerId, ranking]));
  const projectionByPlayer = new Map(input.projections.map((projection) => [projection.playerId, projection]));
  ...
  return input.players
    ...
    .map((player) => {
      ...
      const noteBoost = note?.tag === "target" ? 6 : note?.tag === "avoid" ? -12 : 0;

      return {
        player,
        score: Math.round(valueScore + needScore + scarcityScore + strategyEvaluation.score - byeRisk + noteBoost),
        valueScore,
        needScore,
        scarcityScore,
        ...
        byeRisk,
        notes: [
          ...buildRecommendationNotes(player, ranking, projection, note, needScore, scarcityScore, byeRisk),
...
function scoreNeed(player: Player, rosterCounts: Map<string, number>, roster: Roster | undefined, settings: LeagueSettings): number {
  ...
  const flexSlots = player.positions.some((position) => FLEX_POSITIONS.has(position))
...
function countRosterPositions(roster: Roster, playersById: Map<string, Player>): Map<string, number> {
...
function scoreScarcity(player: Player, remainingByPosition: Map<string, number>): number {
  const primaryPosition = player.positions[0];
...
function scoreByeRisk(player: Player, roster: Roster | undefined, rosterByeCounts: Map<number, number>): number {
...
function countRemainingByPosition(players: Player[], draftedPlayerIds: Set<string>): Map<string, number> {
  const counts = new Map<string, number>();
...
function countRosterByeWeeks(roster: Roster, playersById: Map<string, Player>): Map<number, number> {
