...
function getInferredDepthCharts(players: Player[]): StrategyContext["inferredDepthCharts"] {
...
export function evaluatePlayerStrategy(player: Player, context: StrategyContext | undefined): StrategyEvaluation {
  ...
  const playerContribution = findPlayerContribution(player, positionProfile?.playerContributions);
  const playerContributionShare = playerContribution?.fantasyPointShare ?? playerContribution?.opportunityShare;
  const notes: string[] = [];
  let score = 0;
  ...
  if (context.strategy.preferTeamPointEngines && teamPositionShare) {
    ...
    if (teamPositionShare >= context.strategy.highValuePositionShare) {
      notes.push(`${player.team} generates a high share of points through ${primaryPosition}`);
    }
  }

  if (positionProfile?.opportunityGrade) {
    score += positionProfile.opportunityGrade * context.strategy.weights.teamPositionValue * 0.5;
  }
  ...
  if (depthChartRank) {
    ...
    if (depthChartRank === 1) {
      ...
      notes.push(`${player.team} ${primaryPosition}${depthChartRank} with depth-chart upside`);
    } else if (depthChartRank >= 5 && context.strategy.penalizeBuriedDepthChartPlayers) {
      notes.push(`Buried on ${player.team} ${primaryPosition} depth chart`);
    }
  }

  if (playerContributionShare) {
    score += playerContributionShare * context.strategy.weights.playerContribution;
    notes.push(`${Math.round(playerContributionShare * 100)}% individual contribution signal`);
  }

  if (playerContribution?.trend === "rising" && context.strategy.boostRisingUsage) {
...
export function summarizeTeamStrategy(player: Player, context: StrategyContext | undefined): string | undefined {
  if (!context || !player.team) {
    return undefined;
  }
  ...
  const depthChartRank = findDepthChartRank(player, context, context.teamProfiles[player.team]);
...
function inferDepthCharts(players: Player[]): StrategyContext["inferredDepthCharts"] {
  ...
  for (const player of players) {
    ...
    const teamPositions = teamDepthCharts.get(player.team) ?? new Map<Position, Player[]>();
    teamPositions.set(position, [...(teamPositions.get(position) ?? []), player]);
    ...
    for (const [position, positionPlayers] of teamPositions) {
      teamPositions.set(
        ...
        [...positionPlayers].sort(
          (a, b) => (a.searchRank ?? Number.MAX_SAFE_INTEGER) - (b.searchRank ?? Number.MAX_SAFE_INTEGER)
        )
      );
    }
  }
...
function findDepthChartRank(
  ...
  const primaryPosition = player.positions[0];
  const manualEntry = teamProfile?.positions[primaryPosition]?.depthChart?.find((entry) => {
    if (entry.playerId) {
  ...
  const inferredDepthChart = player.team ? context.inferredDepthCharts.get(player.team)?.get(primaryPosition) : undefined;
  const inferredIndex = inferredDepthChart?.findIndex((candidate) => candidate.id === player.id) ?? -1;
...
function findPlayerContribution(
  ...
  return contributions?.find((contribution) => {
    ...
    return contribution.playerName ? normalizeName(contribution.playerName) === normalizeName(player.fullName) : false;
  });
}
