...
import type { DraftRecommendation, NormalizedLeagueData } from "../../domain/types";
...
import { getVisiblePositions, groupRecommendationsByPosition } from "./draftPositionUtils";
...
export function DraftPickHelperModule({
  ...
  const bestAvailable = [...recommendations].sort((a, b) => b.score - a.score).slice(0, 5);
  const shouldShowPickHistory = picks.length > 0;
  const suppressHoverDetailsAfterScroll = useCallback(() => {
    hoverDetailsSuppressedUntil.current = Date.now() + HOVER_DETAIL_SCROLL_COOLDOWN_MS;
  }, []);

  return (
    <section className={isMinimized ? "panel module-panel module-is-minimized" : "panel module-panel"}>
      <ModuleTrimToggle isMinimized={isMinimized} moduleName="Draft Room" onToggle={onToggleMinimized} />
      <div className="panel-header">
        <div className="draft-room-intro">
          <div className="draft-room-heading">
            <h2>Draft Room</h2>
            <div className="draft-room-status">
              <StatusChip label="Phase" value={formatDraftStatus(data.draft?.status)} />
              <StatusChip label="Picks" value={String(picks.length)} />
              <StatusChip label="Your picks" value={String(myPicks.length)} />
              <StatusChip label="Board pool" value={availablePlayerCount.toLocaleString()} />
            </div>
            <p className="draft-room-helper">Hover a player tile for full recommendation details.</p>
          </div>

          {!isMinimized ? <BestAvailableStrip recommendations={bestAvailable} /> : null}
        </div>
      ...
      {!isMinimized ? (
        <div className="draft-room-layout">
          <div className="draft-board-scroll" aria-label="Draft recommendations by position">
            <div className="draft-board-grid">
              {visiblePositions.map((position) => (
                <DraftPositionColumn
                  ...
                  position={position}
                  recommendations={recommendationsByPosition.get(position) ?? []}
                />
              ))}
            </div>
          </div>
