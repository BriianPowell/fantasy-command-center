...
import type { TeamTrackerLineupSlot, TeamTrackerPlayer } from "./teamTrackerModel";

export function TrackerMetric({ label, value }: { label: string; value: string }) {
  return (
...
export function LineupSection({ slots, title }: { slots: TeamTrackerLineupSlot[]; title: string }) {
...
export function RosterSection({ emptyText, players, title }: { emptyText: string; players: TeamTrackerPlayer[]; title: string }) {
  return (
    <section className="team-roster-section">
      <header>
        <h3>{title}</h3>
      ...
      <div className="team-player-list">
        {players.length ? players.map((player) => <TeamPlayerRow key={player.id} player={player} />) : <p>{emptyText}</p>}
      </div>
    </section>
  );
}

export function RecentTeamPicksSection({ picks, players }: { picks: DraftPick[]; players: Player[] }) {
...
function TeamPlayerRow({ player, slotLabel }: { player: TeamTrackerPlayer; slotLabel?: string }) {
  ...
  return (
    <PlayerReferenceTile
      ...
      className={player.isDraftAddition ? "team-player-row draft-addition" : "team-player-row"}
      leadingLabel={<SlotBadge slotLabel={slotLabel ?? player.primaryPosition} />}
      meta={[
